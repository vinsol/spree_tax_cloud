...
*/
  ...
  var fieldID = "tax_category_tic", fieldName = $('#' + fieldID).attr('name'); 
...
$(document).ready(function () {
    ...
    if (checkRequirements(reqs)) {
        ...
        $(jqReplace).replaceWith('<span style="' + rescss + '" id="' + catListID + '"></span><span style="' + rescss + '" id="' + ticFinalID + '">[Loading...]</span><span id="' + ticCompleteID + '"></span>')
        $('#catList').html('');
...
function showTic() {
    if (ct.length != 5) {
        ...
        $('#catList').html("");$('#jqTicFinal').html('<select style="' + ddlcss + '" id="' + fieldID + '" onchange="getTic(this)"><option>[ Loading... ]</option></select>');
        ...
        $('#catList').html(""); $('#jqTicFinal').html("<input type='hidden' id='" + fieldID + "' value='" + ct +  "' />"); $('#ticComplete').html("");
...
function saveTic() {
    ...
    if (sm.toUpperCase() == "GET") {
        $.get(st, { fieldID: newTic, itemIDField: itemID }, function() { $('#ticComplete').html("<i>Saved</i>"); ct = newTic; showTic(); callbackFromSaveTic()});
    }else{
        $.post(st, { fieldID: newTic, itemIDField: itemID }, function() { $('#ticComplete').html("<i>Saved</i>"); ct = newTic; showTic(); callbackFromSaveTic() });
    } ;
}
...
function getTic(whichTic) {
    if (whichTic.value == 'reset') {
        ...
        if (selectedObj.tic.children) {
            ...
            $('#catList').append(selectedLabel);
            $('#jqTicFinal').html("<input type='hidden' id='" + fieldID + "' value='" + selectedValue +  "'name='" + fieldName + "'/>");
        }
    }
}
